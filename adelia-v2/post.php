...
// Behavior / Appearance
// Removed captcha option since it's unused
define('CHESSIB_REQMOD', "disable"); // To enable moderation change this to "all" or "files"
define('CHESSIB_THREADSPERPAGE', 5);
define('CHESSIB_PREVIEWREPLIES', 3);
// Now using character count truncation (900 characters) instead of 15 lines
define('CHESSIB_TRUNCATE', 900);

// Flood / Limit
define('CHESSIB_DELAY', 5);             // Seconds between posts from same IP
...
function buildPost($post, $respage = false) {
    ...
    // If not in reply mode (main board view), truncate long posts to CHESSIB_TRUNCATE characters.
...
// Pagination links are added above and below the threads.
...
function rebuildThread($id) {
    ...
    $html = "";
    foreach ($posts as $p) {
        // In reply mode, pass true so that buildPost() omits the [Reply] link.
...
if (isset($_POST['body']) || isset($_FILES['file'])) {
    ...
    // Read parent (0 = new thread, else reply)
    $parent = 0;
...
if ($redirect) {
    $redirect_url = ($parent != 0) ? "res/" . $parent . ".html?" . time() . "#{$postid}" : "index.html?" . time();
